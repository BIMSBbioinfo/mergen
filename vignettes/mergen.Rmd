...
For setting up an agent for the openai API platform, you can make use of the `setupAgent` function.
...
the `setupAgent` function returns a list containing all the agent information which is can be used by subsequent functions. Important to note is that the ai_api_key should contain your openai API key, given as a string. 

#### setting up an agent for other replicate
`setupAgent` also contains functionality for setting up an agent for replicate AIs. Let's look at how we then use this function:
